...
class ImbuesTablePolicy extends Policy
{
    public function scopeIndex(User $identity, Query $query): void
    {
        if (!$this->hasAuth(Authorization::ReadOnly)) {
            $query->where(['Characters.plin' => $this->getPlin()])
              ->leftJoin(
                  ['CharactersImbues' => 'characters_imbues'],
                  ['CharactersImbues.imbue_id' => 'Imbues.id'],
              )
              ->leftJoin(
                  ['Characters' => 'characters'],
                  ['Characters.id' => 'CharactersImbues.character_id'],
              );
        }
    }
}
